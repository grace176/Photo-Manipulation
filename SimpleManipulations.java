...
public class SimpleManipulations {
    ...
    public static PixelPicture border(
        ...
        for (int row = 0; row < h + 2 * borderWidth; row++) {
            for (int col = 0; col < borderWidth; col++) {
                tgt[row][col + borderWidth + w] = borderColor;
            }
        }
    ...
    public static PixelPicture vignette(PixelPicture pic) {
        ...
        // coordinates of the center pixels of the image
        double cx = (w - 1) / 2.0;
        double cy = (h - 1) / 2.0;
        ...
        for (int col = 0; col < w; col++) {
            for (int row = 0; row < h; row++) {
                ...
                bmp[row][col] = new Pixel(
                        (int) Math.round(bmp[row][col].getRed() * f),
                        (int) Math.round(bmp[row][col].getGreen() * f),
                        (int) Math.round(bmp[row][col].getBlue() * f)
                );
